...
class Info extends MagentoPaymentInfo
{
    ...
    protected function _prepareSpecificInformation($transport = null)
    {
        ...
        if ($additionalInformation = $this->getInfo()->getAdditionalInformation()) {
            if ( isset($additionalInformation['raw_details_info']['source_type']) && !empty($additionalInformation['raw_details_info']['source_type'])) {
                $data['Type'] = $additionalInformation['raw_details_info']['source_type'];
            }
            if ( isset($additionalInformation['raw_details_info']['source_card_type']) && !empty($additionalInformation['raw_details_info']['source_card_type'])) {
                $data['Card type'] = $additionalInformation['raw_details_info']['source_card_type'];
            }
            if ( isset($additionalInformation['raw_details_info']['source_masked_card']) && !empty($additionalInformation['raw_details_info']['source_masked_card'])) {
                $data['Card'] = $additionalInformation['raw_details_info']['source_masked_card'];
            }
            if ( isset($additionalInformation['raw_details_info']['source_exp_date']) && !empty($additionalInformation['raw_details_info']['source_exp_date'])) {
                $data['Exp date'] = $additionalInformation['raw_details_info']['source_exp_date'];
            }
            if ( isset($additionalInformation['raw_details_info']['state']) && !empty($additionalInformation['raw_details_info']['state'])) {
